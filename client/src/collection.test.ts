...
describe('BibleCollection', () => {
    ...
    it("sanitize_reference", ({expect}) => {
        ...
        // Valid basic args
        expect(collection.sanitize_reference('2th')).toEqual(
            {book: '2th', start_chapter: 1, start_verse: 1, end_chapter: 1, end_verse: 1})
        expect(collection.sanitize_reference('2th', 1)).toEqual(
            {book: '2th', start_chapter: 1, start_verse: 1, end_chapter: 1, end_verse: 1})
        expect(collection.sanitize_reference('2th', 1, 1)).toEqual(
            {book: '2th', start_chapter: 1, start_verse: 1, end_chapter: 1, end_verse: 1})

        // Invalid basic args
        expect(collection.sanitize_reference('invalid')).toEqual(
            {book: 'gen', start_chapter: 1, start_verse: 1, end_chapter: 1, end_verse: 1})
        expect(collection.sanitize_reference('2th', -1)).toEqual(
            {book: '2th', start_chapter: 1, start_verse: 1, end_chapter: 1, end_verse: 1})
        expect(collection.sanitize_reference('2th', 99)).toEqual(
            {book: '2th', start_chapter: 3, start_verse: 18, end_chapter: 3, end_verse: 18})
        expect(collection.sanitize_reference('2th', 2, -1)).toEqual(
            {book: '2th', start_chapter: 2, start_verse: 1, end_chapter: 2, end_verse: 1})
        expect(collection.sanitize_reference('2th', 2, 99)).toEqual(
