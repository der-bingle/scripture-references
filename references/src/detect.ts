...
const regex_additional_range = '(?<= ? ?[,;] ? ?)' + regex_verse_range + regex_trailing
...
export function* detect_references(text:string,
    ...
    while (true){
        ...
        if (ref && ref.args_valid){
            ...
            const add_regex = new RegExp(regex_additional_range, 'uig')
            ...
            while (true){
                ...
                let prefix = ref.book
                const has_verse_sep = new RegExp(regex_verse_sep).test(add_match[0])
                if (!has_verse_sep && ['verse', 'range_verses', 'range_multi'].includes(ref.type)){
                    prefix += `${ref.end_chapter}:`
                }
                const add_ref = PassageReference.from_string(prefix + add_match[0], book_names)
                ...
                yield {
                    ref: add_ref,
                    text: add_match[0],
                    index: add_match.index,
                    index_from_prev_match: add_match.index - end_of_prev_match,
                }
                end_of_prev_match = add_match.index + add_match[0].length
            }

            // Move main regex up to where additional ranges regex got up to
