...
describe('clean_note', () => {
    ...
    it('should prefix all classes with fbn-', ({expect}) => {
        const result = clean_note('<span class="greek">greek words</span>')
        expect(result).toEqual('<span class="fbn-greek">greek words</span>')
    })
    ...
    it('should transform non standard markup as HTML', ({expect}) => {
        ...
        expect(result).toEqual('The Lord <em class="fbn-sc">Jesus</em> is King')
        result = clean_note('The Lord <span class="bold-sc">Jesus</span> is King')
        expect(result).toEqual('The Lord <strong class="fbn-sc">Jesus</strong> is King')
        ...
        expect(result).toEqual('The Lord <em class="fbn-yhwh">Jesus</em> is King')
        result = clean_note('The Lord <span class="sn-excerpt-divine-name">Jesus</span> is King')
        expect(result).toEqual('The Lord <span class="fbn-excerpt fbn-yhwh">Jesus</span> is King')
        result = clean_note('The year 2300 <span class="bold-era">AD</span> it')
        expect(result).toEqual('The year 2300 <strong class="fbn-era">AD</strong> it')
        result = clean_note('<span class="sn-hebrew-chars">א</span>')
        expect(result).toEqual('<span class="fbn-hebrew">א</span>')
        result = clean_note('<span class="sn-ref-sc">title</span> This psalm was originally')
        expect(result).toEqual('<span class="fbn-sc">title</span> This psalm was originally')
        ...
        expect(result).toEqual('<span class="fbn-list-1">Reasons to be a geek</span>')
        result = clean_note('<span class="sn-list-2">Collections</span>')
        expect(result).toEqual('<span class="fbn-list-2">Collections</span>')
        result = clean_note('<span class="sn-list-3">Movies</span>')
        expect(result).toEqual('<span class="fbn-list-3">Movies</span>')
        result = clean_note('In <span class="sn-excerpt">the beginning God</span> created')
        expect(result).toEqual('In <span class="fbn-excerpt">the beginning God</span> created')
        result = clean_note('lots were called <span class="sn-excerpt-roman">purim</span>')
        expect(result).toEqual('lots were called <span class="fbn-excerpt fbn-roman">purim</span>')
        result = clean_note('<span class="sn-excerpt-sc">Mene, Mene, Tekel,</span> and')
        expect(result).toEqual('<span class="fbn-excerpt fbn-sc">Mene, Mene, Tekel,</span> and')
    })
